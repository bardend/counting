...
def detect(out, image):

    tensor = tf.convert_to_tensor(out)

    yolo_outputs = yolo_head(tensor, anchors, len(class_names))
    out_scores, out_boxes, out_classes=yolo_eval(yolo_outputs, [image.size[1],  image.size[0]], 10, 0.5, 0.5)

    colors = get_colors_for_classes(len(class_names))
    return draw_bboxes(image, out_boxes, out_classes, class_names, out_scores)
